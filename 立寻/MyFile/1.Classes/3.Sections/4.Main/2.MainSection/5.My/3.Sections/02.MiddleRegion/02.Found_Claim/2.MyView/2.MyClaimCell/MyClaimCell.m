...
-(instancetype)initWithDictionary:(NSDictionary *)dictionary andHeight:(float)height andDelegate:(id)delegate andIndexPath:(NSIndexPath *)indexPath{
    if (self = [super init]) {
        ...
        //头像
        {
            float top_height = 40/195.0*height;//第一个分割线y坐标
            ...
            [MYTOOL setImageIncludePrograssOfImageView:imgV withUrlString:dictionary[@"user_url"]];
            left = 10 + imgHeight + 10;
        }
        //用户昵称
        {
            UILabel * label = [UILabel new];
            label.text = dictionary[@"user_nick"];
            label.font = [UIFont systemFontOfSize:13];
            label.textColor = MYCOLOR_48_48_48;
            CGSize size = [MYTOOL getSizeWithLabel:label];
            label.frame = CGRectMake(left, 40/195.0*height/2.0 - size.height/2, size.width, size.height);
        ...
        //认领时间
        {
            ...
            {
                UILabel * label = [UILabel new];
                label.text = [NSString stringWithFormat:@"认领时间：%@",dictionary[@"extensionTime"]];
                label.font = [UIFont systemFontOfSize:10];
                label.textColor = [MYTOOL RGBWithRed:144 green:144 blue:144 alpha:1];
                [self addSubview:label];
                CGSize size = [MYTOOL getSizeWithLabel:label];
                label.frame = CGRectMake(WIDTH-10-size.width, 40/195.0*height/2 - size.height/2, size.width, size.height);
                [self addSubview:label];
            ...
            //图标
            {
                ...
                icon.frame = CGRectMake(left-12-5, 40/195.0*height/2-6, 12, 12);
